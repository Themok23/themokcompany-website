...
export default function VentureClient({ slug }: { slug: string }) {
    ...
    return (
      <div className="w-full text-white min-h-screen flex items-center justify-center relative z-[1]">
        <div className="text-center">
          <h1 className="text-4xl font-light mb-4 font-heading">Venture Not Found</h1>
          <Link href="/ventures" className="text-primary hover:text-white transition-colors">
            Back to Ventures
          </Link>
        </div>
      </div>
  ...
  const statusLabel = venture.status === "active" ? "Active" : venture.status === "launched" ? "Launched" : venture.status === "coming-soon" ? "Coming Soon" : "Stealth";
    ...
    <div className="w-full text-white min-h-screen overflow-x-hidden relative z-[1]">
      <section className="pt-40 pb-12 md:pt-48 md:pb-16 px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <Link href="/ventures" className="inline-flex items-center gap-2 text-muted hover:text-primary transition-colors mb-8 font-body">
      ...
      <section className="border-b border-border py-16 px-6 lg:px-8">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          {prevVenture ? (
            <Link href={`/ventures/${prevVenture.slug}`} className="group flex items-center gap-3 text-muted hover:text-white transition-colors">
          ...
          {nextVenture ? (
            <Link href={`/ventures/${nextVenture.slug}`} className="group flex items-center gap-3 text-muted hover:text-white transition-colors text-right">
              <div>
